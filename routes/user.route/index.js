...
const {
  ...
  ensureAuthenticatedForUser,
} = require("../../config/auth");
require("../../config/passport.conf")(passport);
// Login Page

// Register Page
router.get("/account", forwardAuthenticated, (req, res) =>
  res.render("register", { title: "ticketMe| Tài khoản" })
);
...
router.post("/cancle", async (req, res) => {
  let { MaVeXe, MaCX } = req.body;
  // let Email = req.user.email;
  const seat = await UserMysql.findSeatByEmail(MaVeXe, req.user.email);
  if (!seat) return res.status(400).json({ is: false });

  UserMysql.getPostDetailed(MaCX, (result) => {
    if (result.length) {
      const diff =
        new Date().getTime() - new Date(result[0].NgayDi).getTime() + 86400000;
      if (diff < 0)
        UserMysql.destroyTicket(MaVeXe, async (result) => {
          if (result) {
            return Cancle.sendMailForCancle(req, res, MaVeXe)
            // return res.status(200).send({ is: true });
          } else {
            res.status(400).json({ is: false });
          }
        });
      else return res.status(400).json({ is: false });
    }
  });
});
...
router.post("/me/pass", ensureAuthenticated, async (req, res) => {
  ...
  if (req.body.new == "" || req.body.new.length < 6) {
    req.flash("error_msg", "Mật khẩu không đủ dài");
  ...
  User.findOne({ email: req.body.email }).then((user) => {
    ...
    const output = `
        ...
        <ul>
          ...
          <li>Secret code:<strong>${req.body.check}</strong></li>
...
router.post("/me/email", (req, res) => {
  emailExistence.check(req.body.email, function (error, response) {
    if (response) {
      ...
      User.findOne({ email: req.body.email }).then((user) => {
        if (user) {
          req.flash("error_msg", "Email đã được đăng kí");
          return res.redirect("/user/me");
        } else {
          if (req.body.password == req.user.password) {
            ...
            req.flash("error_msg", "Xác nhận mật khẩu không đúng");
            return res.redirect("/user/me");
          }
        }
      });
...
router.post("/account/register", (req, res) => {
  ...
  if (!name || !email) {
    req.flash("error_msg", " Không được bỏ trống");
  ...
  emailExistence.check(email, function (error, response) {
    ...
    if (response) {
        ...
        if (user) {
          ...
          bcrypt.genSalt(10, (err, salt) => {
            newUser
              .save()
              .then((user) => {
                req.flash("success_msg", "Bây giờ, bạn có thể đăng nhập");
      ...
      req.flash("error_msg", "Email này không hợp lệ");
      res.redirect("/user/account#dangki");
    }
  });
});
...
router.post("/account/login", (req, res, next) => {
  ...
  passport.authenticate("local", function (err, user, info) {
    ...
    req.logIn(user, function (err) {
      ...
      console.log(req.body)
      return res.status(200).json({ ok: true, path: req.body.redirect || '/home'})
    });
  })(req, res, next);
});
