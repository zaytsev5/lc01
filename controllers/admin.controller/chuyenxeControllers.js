...
var ChuyenXe = require("../../services/admin_services/chuyenxeModels.js");
...
exports.createChuyenXe = function (req, res) {
  ...
  if (!newChuyenXe.MaCX) {
    ...
    ChuyenXe.createChuyenXe(newChuyenXe, function (err, chuyenxe) {
      ...
      return res.json({ status: true, data: chuyenxe });
    });
  }
};
...
exports.updateChuyenXeById = function (req, res) {
  ChuyenXe.updateChuyenXeById(
    req.params.MaCX,
    new ChuyenXe(req.body),
    function (err, chuyenxe) {
      if (err) res.send(err);
      res.json(chuyenxe);
    }
  );
