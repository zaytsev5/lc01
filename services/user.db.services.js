...
module.exports = {
  ...
  findSeatByEmail: function (seat, email) {
    let query = "select MaVeXe from hoadon where Email=? and MaVeXe=?";
    // let bind = [];
    // bind.push(postID);
    return new Promise((resolve, reject) => {
      mysqlDB.query(query, [email, seat], (err, result) => {
        if (err) return reject({status:false});
        resolve(result)
      });
    })
  },
  createPost: function (posts) {
    let query = "insert into `chuyenxe`(`MaCX`,`MaTX`,`BienSoXe`, `NgayDi`, `GioDi`) values (?,?,?,?,?)";
    // bind.push(postID);
    return new Promise((resolve, reject) => {
      for(let i = 0; i < posts.length; i++) {
        let bind = [];

        bind.push(posts[i].MaCX)
        bind.push(posts[i].MaTX)
        bind.push(posts[i].BienSoXe)
        bind.push(posts[i].NgayDi)
        bind.push(posts[i].GioDi)
  ...
  updateCus: function (cus, email, callback) {
    console.log(cus, email)
    let query =
    "update `khachhang` set  `TenKH`= ?, `SDT`= ?, `DiaChi` =? where Email=?";
    cus.push(email);
    mysqlDB.query(query, cus, (err, result) => {
      if (err) {
        console.log(err.message)
  ...
  getPostDetailed: async function (postID, callback) {
    ...
    return mysqlDB.query(query, postID, (err, result) => {
      if (err) return console.log(err.message);
      callback(result);
    });
  },
  getInfoPostForMail: async function (postID) {
    let query = `SELECT *
	FROM chuyenxe
	INNER JOIN tuyenxe ON chuyenxe.MaTX =  tuyenxe.MaTX
	INNER JOIN xe ON chuyenxe.BienSoXe = xe.BienSoXe where chuyenxe.MaCX = ?`;
