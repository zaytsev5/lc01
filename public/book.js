...
buttonCancle.addEventListener("click", () => {
  ...
  if (quit) return (window.location = base_url);
  return;
});
...
timer.addEventListener("change", () => {
  ...
  seatNums = 0;
  console.log(
    new Date(inputDate.value).toLocaleDateString(),
  ...
  if (new Date(inputDate.value).getTime() <= new Date().getTime()) {
    if (
      ...
      timer.value.slice(0, 2) - new Date().getHours() >= 2
    ) {
...
function checkPostByURL() {
  if (window.location.search.slice(3, window.location.search.length) != "") {
    inputDate.value = window.location.search.slice(
      3,
...
async function findPostDate() {
  ...
  try {
    ...
    if (myJson.length != 0) {
      $(".date-go-detail").html(
        new Date(myJson[0].NgayDi).toLocaleDateString()
      );
      fillTimePicker();
    } else {
...
async function fillTimePicker() {
  ...
  let dateTrip = inputDate.value;
  try {
    const response = await fetch(`${base_url}/time/${tripID}/${dateTrip}`);
    const myJson = await response.json();
...
async function findPostTime() {
  ...
  try {
    const response = await fetch(
      `${base_url}/post/${tripID}/${dateTrip}/${time}`
    );
    const myJson = await response.json();
    ...
    //GET NOIXUATPHAT
    const post = await fetch(`${base_url}/getPostDetails/${postSelected}`);
    const postDetails = await post.json();
    const numberOfSeats = postDetails[0].SoLuongGhe;
    const type = postDetails[0].LoaiXe;
    if (numberOfSeats < 40) {
    }
    // if(numberOfSeats == 30 ) {
    //   const seatMap = document.querySelector('.s30')
    //   seatMap.parentElement.removeChild(seatMap)
    //   document.querySelector('.s40').style.display = 'flex'
    // }
    let seatmap = `<div id="seat-map-columns">`;
    for (let i = 1; i <= numberOfSeats; i++) {
      let seat = ("0" + i).slice(-2);
      let seat_name = `A${seat}`;

      if (type === "VIP" && i > 20) seat_name = `B${("0" + (i - 20)).slice(-2)}`;

      seatmap += `<button class="seat" id="${seat_name}">${seat_name}</button>`;
      if (i % 10 == 0) {
        seatmap += "</div>";
        if (i !== numberOfSeats) {
          seatmap += '<div id="seat-map-columns">';
        }
      }
    }
    ...
    // }
    document.querySelector(".flex-col").innerHTML = seatmap;
    seatPicker();
...
async function addCustomerAndTicket(event) {
  ...
  // VALIDATION
  const numbericRegex = new RegExp("^[0-9]{10,11}$");
  ...
  if (
    $("#email").val().length <= 0 ||
    emailRegex.exec($("#email").val()) === null
  ) {
  ...
  if (
    $("#phone").val().length <= 0 ||
    numbericRegex.exec($("#phone").val()) === null
  ) {
    event.target.disabled = false;
  ...
  try {
    let response = await fetch(`${base_url}/checkseat/${postSelected}`);
    ...
    fetch(`${base_url}/customer/insert`, {
      method: "POST",
      headers: {
    ...
    btn.addEventListener("click", (e) => {
      if (e.target.className.includes("selected")) {
        ...
        for (var i = 0; i < tickets.length; i++) {
          if (tickets[i] === btn.id) tickets.splice(i, 1);
        }
